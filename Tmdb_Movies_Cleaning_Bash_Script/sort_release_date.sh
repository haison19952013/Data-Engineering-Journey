...
header=$(head -n 1 tmdb-movies.csv)
...
}' tmdb-movies.csv >> "$temp_file"

# Extract header for the final file
head -n 1 "$temp_file" > sorted-tmdb-movies.csv

# Sort the data (skip header) by date in reverse order and append to final file
tail -n +2 "$temp_file" | grep -v "^$" | sort -t, -k1,1r >> sorted-tmdb-movies.csv

# Clean up temporary file
rm "$temp_file"

echo "Processing complete. Results saved to sorted-tmdb-movies.csv"